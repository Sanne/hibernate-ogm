...
public class InfinispanRemoteDialect<EK,AK,ISK> extends BaseGridDialect implements MultigetGridDialect {
	...
	public void insertOrUpdateTuple(EntityKey key, Tuple tuple, TupleContext tupleContext) {
		...
		final long knownVersion = versionedTuple.getVersion();
		final String cacheName = key.getTable();
...
//			optimisticLockFailed = ! mapper.withinCacheEncodingContext( c -> c.replaceWithVersion( idBuffer, valuePayload, knownVersion ) );
//			if ( optimisticLockFailed ) {
//				//Note: currently OGM temporarily removes an entity to update its relations, and this removal causes the optimistic replace to fail.
//				//At this point in code we can't know why the replace failed, so we need to try a _putIfAbsent_
//				optimisticLockFailed = null != mapper.withinCacheEncodingContext( c -> c.putIfAbsent( idBuffer, valuePayload ) );
//				if ( optimisticLockFailed ) {
//					throw new StaleObjectStateException( "TBD", null );
//				}
//				else {
//					versionedTuple.setVersion( 1l );
//				}
//			}
//			else {
//				versionedTuple.setVersion( knownVersion + 1 );
//			}
		}
	}
